...
- **Notes:**
  - **Imports:**
    ...
    - store: `from infn_jobs.store.schema import init_db`
    - pipeline: `from infn_jobs.pipeline.curate import rebuild_curated`
    - store export: `from infn_jobs.store.export.csv_writer import export_all`
  - **`execute()` implementation:**
    ```python
    conn = sqlite3.connect(str(DB_PATH))
    try:
  ...
  - `rebuild_curated(conn)` refreshes `calls_curated` and `position_rows_curated` from the latest `calls_raw` and `position_rows` data before writing CSVs. This ensures the curated CSVs always reflect the current DB state.
...
- `python3 -m infn_jobs export-csv` — completes; `$?` is 0; `data/exports/` contains `calls_raw.csv`, `calls_curated.csv`, `position_rows_raw.csv`, `position_rows_curated.csv`.
