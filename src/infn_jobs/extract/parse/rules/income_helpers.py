...
TOTAL_RE = re.compile(r"\btotale\b", re.IGNORECASE)
...
def find_amount(
    *,
    ...
    for line in iter_lines(segment_text):
        label_match = label_re.search(line)
        if label_match is None:
            continue
        if require_total and TOTAL_RE.search(line) is None:
            continue
        if require_yearly and YEARLY_RE.search(line) is None:
            continue
        if require_monthly and MONTHLY_RE.search(line) is None:
            continue
        if require_no_qualifier and not line_has_no_qualifier(line):
            continue
        amount = extract_amount(line, start_pos=label_match.end())
        if amount is None:
            continue
        candidate = (amount, line)
    return candidate
